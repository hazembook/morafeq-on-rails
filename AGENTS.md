...
- **Secret files**: Do NOT read, grep, or glob `.kamal/secrets*`, `config/*.key`, or `config/credentials.yml.enc`. These are blocked by opencode.json permissions for enforcement. Only `.example` counterparts are safe.
